#!/usr/bin/env python3
"""code-rust-crate-attestation-oracle -- a deterministic attestation-integrity decider for the `code-rust-crate` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `code-rust-crate` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='code-rust-crate', a non-empty
...
def decide(c):
    if c.get("domain") != "code-rust-crate": return "INVALID"
...
def _run_probes(path):
    ...
    print("code-rust-crate attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
